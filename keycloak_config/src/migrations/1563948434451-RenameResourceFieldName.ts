...
export class RenameResourceFieldName1563948434451
  ...
  public async up(queryRunner: QueryRunner): Promise<any> {
    ...
    await queryRunner.query(
      'ALTER TABLE `keycloak_config` ADD `created_at` datetime(6) NULL DEFAULT CURRENT_TIMESTAMP(6)'
    );
    await queryRunner.query(
      'ALTER TABLE `keycloak_config` ADD `updated_at` datetime(6) NULL DEFAULT CURRENT_TIMESTAMP(6)'
    );
  }

  public async down(queryRunner: QueryRunner): Promise<any> {
    await queryRunner.query(
      'ALTER TABLE `keycloak_config` DROP COLUMN `updated_at`'
    );
    await queryRunner.query(
      'ALTER TABLE `keycloak_config` DROP COLUMN `created_at`'
    );
    await queryRunner.query(
